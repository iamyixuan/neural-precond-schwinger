...
def run(job: RunningJob):
    ...
    model = PrecondCNN(
        ...
        n_layers_gauge=config["num_layers"],
        n_layers_precond=config["num_layers"],
    ...
    loss_name = "CNNMatCondNumberLoss"
    ...
    try:
        for epoch in tqdm(range(5)):
            ...
            for i, (X, y) in enumerate(train_loader):
                ...
                optimizer.zero_grad()
                y_pred = model(X)
                loss = loss_fn(y, y_pred, scale=model.scale)  # unsupervised loss
                loss.backward()
            ...
            with torch.no_grad():
                ...
                for i, (X, y) in enumerate(val_loader):
                    X, y = X.to(device), y.to(device)
                    y_pred = model(X)
                    val_loss += loss_fn(y, y_pred, scale=model.scale)
