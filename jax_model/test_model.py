...
def main(args, configs):
    log_dir = f"./logs/FNO_L{args.model_L}_{args.loss}Precond_invloss/"
    model = load_model(configs, FNO2d, log_dir)

    if args.data_L == 8:
        # data_name = "config.l8-N200-b2.0-k0.276-unquenched-test.x.npy"
        data_name = "config.l8-N200-b2.0-k0.276-unquenched-new.npz"
    elif args.data_L == 16:
    ...
    jnp.savez(
        f"./plot_data/FNO_L{args.model_L}_D{args.data_L}_{args.loss}Precond_unpc_hist.npz",
        hist=no_pc_hist,
        time=no_pc_time,
    )
    jnp.savez(
        f"./plot_data/FNO_L{args.model_L}_D{args.data_L}_{args.loss}Precond_nn_pc_hist.npz",
        hist=nn_pc_hist,
        time=nn_pc_time,
    )
    jnp.savez(
        f"./plot_data/FNO_L{args.model_L}_D{args.data_L}_{args.loss}Precond_ic_pc_hist.npz",
        hist=IC_pc_hist,
        time=IC_pc_time,
    )
    ...
    if args.data_L == 8:
        ...
        precond_mat = get_batch_matrix(f_precond, b_size=U1.shape[0], v_size=2 * X**2)
        ...
        print(f"IC condition number: {q_10_IC:.3f} | {q_50_IC:.3f} | {q_90_IC:.3f}")
        # save the condition number
        jnp.savez(
            f"./plot_data/FNO_L{args.model_L}_D{args.data_L}_{args.loss}Precond_cond_number.npz",
            org=org_cond_number,
            precond=cond_number,
            IC=IC_cond_number,
        )
