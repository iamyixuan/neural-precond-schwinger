...
include("../src/hmc_wilson_forces.jl")
include("../src/hmc.jl")
...
function generating_gauge()
    ...
    nx = 64
    nt = 64
    kappa = 0.276 # Hopping parameter = (2m0 + 4r)^{-1} at 2D where r is Wilson parameter
    mass = (kappa^-1 - 4)/2
    beta = 2.0
    quenched = false
    saving_directory = "/Users/yixuan.sun/.julia/dev/JulianSchwingerModel/app/gauge_configs/"
    ...
    # Initialize angles array: shape (measurements, 2, nx, nt)
    angles_array = zeros(Float64, measurements, 2, nx, nt)
    ...
    while nomeas < hmcparam.measurements
        ...
        if accp == 1
            ...
            # Save angles into the array
            angles_array[nomeas, 1, :, :] = reshape(lattice.anglex, nx, nt)
            angles_array[nomeas, 2, :, :] = reshape(lattice.anglet, nx, nt)
    ...
    NPZ.npzwrite(saving_directory * "config.l$(nx)-N$(measurements)-b$(beta)-k$(kappa)-unquenched.npz", angles_array)
    println("Saved to $(saving_directory)config.l$(nx)-N$(measurements)-b$(beta)-k$(kappa)-unquenched.npz")
end
generating_gauge()
